...
// Helper to convert snake_case from DB to camelCase for our types
...
const mapUser = (user: any): User => ({
  id: user.id,
  email: user.email, // We'll need to fetch email separately or store it. For now, we'll not use email in public.users.
  username: user.username,
  role: user.role,
  createdAt: user.created_at,
  bio: user.bio,
  // securityQuestion/Answer might be used only for password reset; store them if needed
});
...
export const supabaseDb = {
  ...
  async subscribe(subscription: Subscription): Promise<void> {
    // Check if already subscribed (optional, but safe)
  ...
  async unsubscribe(readerId: string, writerId: string): Promise<void> {
    const { error } = await supabase
      .from('subscriptions')
      .delete()
      .eq('reader_id', readerId)
